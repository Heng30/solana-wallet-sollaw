...
pub fn mnemonic_from_phrase(phrase: &str) -> Result<Mnemonic> {
    let mnemonic = Mnemonic::from_phrase(phrase, Language::English)
        .with_context(|| format!("Failed to get mnemonic from phrase: {phrase:?}"))?;

    Ok(mnemonic)
}

pub fn is_valid_mnemonic(mnemonic: &str, mt: MnemonicType) -> bool {
    let words = mnemonic
...
mod tests {
    ...
    fn test_all() -> Result<()> {
        for ty in [MnemonicType::Words12, MnemonicType::Words24] {
            ...
            assert!(is_valid_mnemonic(phrase, ty));
            println!("{phrase}\n");
